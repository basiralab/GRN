...
class GraphConv(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        y = torch.matmul(adj, x.to(device))
        if self.add_self:
            y += x
...
class GcnEncoderGraph(nn.Module):
    ...
    def build_conv_layers(self, input_dim, hidden_dim, embedding_dim, num_layers, add_self,
        ...
        conv_first = GraphConv(input_dim=input_dim, output_dim=hidden_dim, add_self=add_self,
                normalize_embedding=normalize, bias=self.bias).to(device)
        conv_block = nn.ModuleList(
                [GraphConv(input_dim=hidden_dim, output_dim=hidden_dim, add_self=add_self,
                        normalize_embedding=normalize, dropout=dropout, bias=self.bias).to(device)
                 for i in range(num_layers-2)]).to(device)
    ...
    def build_pred_layers(self, pred_input_dim, pred_hidden_dims, label_dim, num_aggs=1):
        ...
        if len(pred_hidden_dims) == 0:
            pred_model = nn.Linear(pred_input_dim, label_dim).to(device)
        else:
            pred_layers = []
            for pred_dim in pred_hidden_dims:
                pred_layers.append(nn.Linear(pred_input_dim, pred_dim).to(device))
                pred_layers.append(self.act)
                pred_input_dim = pred_dim
            pred_layers.append(nn.Linear(pred_dim, label_dim).to(device))
    ...
    def construct_mask(self, max_nodes, batch_num_nodes): 
        ...
        '''
        # masks
        packed_masks = [torch.ones(int(num)).to(device) for num in batch_num_nodes]
        batch_size = len(batch_num_nodes)
        out_tensor = torch.zeros(batch_size, max_nodes).to(device)
    ...
    def gcn_forward(self, x, adj, conv_first, conv_block, conv_last, embedding_mask=None):
        ...
        x_tensor = torch.cat(x_all, dim=2).to(device)
    ...
    def forward(self, x, adj, batch_num_nodes=None, **kwargs):
        ...
        if self.bn:
            x = self.apply_bn(x)
        out_all = []
        out, _ = torch.max(x, dim=1).to(device)
        out_all.append(out)
        for i in range(self.num_layers-2):
            ...
            out_all.append(out)
            if self.num_aggs == 2:
                out = torch.sum(x, dim=1).to(device)
                out_all.append(out)
        ...
        if self.num_aggs == 2:
            ...
            out_all.append(out)
        if self.concat:
            output = torch.cat(out_all, dim=1).to(device)
        else:
            output = out
    ...
    def loss(self, pred, label, type='softmax'):
        # softmax + CE
        if type == 'softmax':
            return F.cross_entropy(pred, label, reduction='mean').to(device)
        elif type == 'margin':
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        # GC
        self.conv_first_after_pool = nn.ModuleList().to(device)
        self.conv_block_after_pool = nn.ModuleList().to(device)
        self.conv_last_after_pool = nn.ModuleList().to(device)
        ...
        self.assign_conv_first_modules = nn.ModuleList().to(device)
        self.assign_conv_block_modules = nn.ModuleList().to(device)
        self.assign_conv_last_modules = nn.ModuleList().to(device)
        self.assign_pred_modules = nn.ModuleList().to(device)
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        ...
        embedding_tensor = self.gcn_forward(x, adj,
                self.conv_first, self.conv_block, self.conv_last, embedding_mask).to(device)
        ...
        for i in range(self.num_pooling):
            ...
            # [batch_size x num_nodes x next_lvl_num_nodes]
            self.assign_tensor = nn.Softmax(dim=-1)(self.assign_pred_modules[i](self.assign_tensor)).to(device)
            if embedding_mask is not None:
                self.assign_tensor = self.assign_tensor * embedding_mask
            ...
            embedding_tensor = self.gcn_forward(x, adj, 
                    ...
                    self.conv_last_after_pool[i]).to(device)
